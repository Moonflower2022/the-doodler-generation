...
class SketchDecoder(nn.Module):
    def __init__(self, hyper_parameters):
        ...
        self.hidden_initiation_linear = nn.Linear(hyper_parameters.LATENT_VECTOR_SIZE, 2 * hyper_parameters.HIDDEN_SIZE)

        self.lstm = nn.LSTM(5, hidden_size=hyper_parameters.HIDDEN_SIZE, dropout=hyper_parameters.DROPOUT)
    ...
    def forward(self, hidden_cell=None):
        start_of_sequence = torch.stack([torch.tensor([0, 0, 1, 0, 0], dtype=torch.float32)] * self.hyper_parameters.BATCH_SIZE).to(self.hyper_parameters.DEVICE)
        # size (100, 5)

        if not hidden_cell:
            hidden = torch.zeros(self.hyper_parameters.HIDDEN_SIZE, dtype=torch.float32).to(device=self.hyper_parameters.DEVICE).unsqueeze(0)
            cell = torch.zeros(self.hyper_parameters.HIDDEN_SIZE, dtype=torch.float32).to(device=self.hyper_parameters.DEVICE).unsqueeze(0)
        ...
        gaussian_parameters, pen_state_logits = stroke_parameters[:, :4], stroke_parameters[:, 4:]
        ...
        return torch.cat([gaussian_sample_x, gaussian_sample_y, pen_state_probabilities], dim=1).to(self.hyper_parameters.DEVICE), hidden_cell

    def generate_stroke(self, last_stroke=None, hidden_cell=None):
        if last_stroke == None:
            last_stroke = torch.tensor([0, 0, 1, 0, 0], dtype=torch.float32).to(self.hyper_parameters.DEVICE)
        ...
        return torch.cat([gaussian_sample_x, gaussian_sample_y, pen_state_probabilities]).to(self.hyper_parameters.DEVICE), hidden_cell
