...
def bivariate_normal_pdf(dx, dy, sigma_x, sigma_y, mu_x, mu_y, rho_xy):
    ...
    bottom = 2*np.pi*sigma_x*sigma_y*torch.sqrt(1-rho_xy**2)
    return top / bottom
...
def vectorized_reconstruction_loss(predictions, labels, logger=None):
    ...
    offset_loss = -torch.sum(torch.log(valid_mask * torch.sum(weighted_gaussian_mixture_probabilities, 2) + HyperParameters.EPSILON))

    logits = nn.functional.softmax(predicted_pen_states, dim=-1)

    pen_state_loss = -torch.sum(label_pen_states * torch.log(logits + HyperParameters.EPSILON))
...
def train_model():
    ...
    gradient_logger = get_logger(f"{__name__}.gradient", f"{folder_name}/gradient.log")
    ...
    for epoch in range(num_epochs):
        ...
        for i, sketch_batch in enumerate(sketches_data_loader):
            ...
            loss = vectorized_reconstruction_loss(outputs, sketch_batch, logger=model_logger)
